#ifndef _SYNCBOSS_FILE_HANDLE_H_
#define _SYNCBOSS_FILE_HANDLE_H_
...
class FileHandle
{
  public:
    ...
    // semantics of open().
    FileHandle(const std::string& path, int flags, mode_t mode = 0) { fd_ = open(path.c_str(), flags, mode); }

    // Constructs a FileHandle by opening |path| relative to |dir_fd|, following
    // the semantics of openat().
    FileHandle(const int directory_fd, const std::string& path, int flags, mode_t mode = 0) { fd_ = openat(directory_fd, path.c_str(), flags, mode); }
    ...
    void Close()
    {
        if (IsValid() && Mode == FileHandleMode::Local) close(fd_);
    ...
    FileHandle Duplicate() const { return FileHandle(Mode == FileHandleMode::Local ? dup(fd_) : fd_); }

    FileHandle<FileHandleMode::Borrowed> Borrow() const { return FileHandle<FileHandleMode::Borrowed>(Get()); }
...
#endif  // _SYNCBOSS_FILE_HANDLE_H_
