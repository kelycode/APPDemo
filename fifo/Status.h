#ifndef _SYNCBOSS_STATUS_H_
#define _SYNCBOSS_STATUS_H_
...
class Status
{
  public:
    ...
    Status(Status&& other) noexcept : value_{std::move(other.value_)}, error_{other.error_} { other.error_ = -1; }
    ...
    {
        std::string message;
        if (error_ > 0) message = ErrorStatus::ErrorToString(error_);
        return message;
    }
...
inline int ReturnStatusOrError(const Status<void>& status) { return status ? 0 : -status.error(); }

inline int ReturnStatusOrError(const Status<int>& status) { return status ? status.get() : -status.error(); }


#endif  // _SYNCBOSS_STATUS_H_
